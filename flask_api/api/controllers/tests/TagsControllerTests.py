...
from LineupOptimizerControllerModule.LineupTagRules import LineupTagRules
...
class TestTagsControllerMethods(unittest.TestCase):
    ...
    def test_all_tags(self):
        ...
        lineup = Lineup(lineup={"QB": josh_allen, "RB1": james_cook, "RB2": tony_pollard, "WR1": stefon_diggs, "WR2": gabe_davis, 
            "WR3": tyler_boyd, "TE": george_kittle, "FLEX": matt_breida, "DST": giants}, site="draftkings")
        self.assertEqual((True, "DST"), LineupTagRules.check_punt_rule(lineup))
        self.assertTrue(LineupTagRules.check_composition_rule(composition={"RB": 3}, lineup=lineup))
        self.assertFalse(LineupTagRules.check_composition_rule(composition={"WR": 4}, lineup=lineup))
        self.assertEqual([(4, 1)], LineupTagRules.check_stack_rule(lineup))

### HELPER METHODS ###

    def get_draftable_by_id(self, playerSiteId: int) -> dict:
        return next((player for player in test_draftables if player["playerSiteId"] == playerSiteId), None)
