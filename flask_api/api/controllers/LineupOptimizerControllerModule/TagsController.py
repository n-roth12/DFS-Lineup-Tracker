...
class TagsController:
    ...
    def get_recommended_tags(lineup: Lineup) -> list:
        ...
        for composition in COMPOSITION_TYPES:
            if LineupTagRules.check_composition_rule(composition=composition,  lineup=lineup):
                result_tags.append({ "category": "Build", "value": f"{list(composition.values())[0]} {list(composition.keys())[0]}" })
