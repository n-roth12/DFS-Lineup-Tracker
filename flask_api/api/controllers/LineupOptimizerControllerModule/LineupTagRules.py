...
class LineupTagRules:
    ...
    def check_stack_rule(lineup: Lineup) -> list:
        ...
        for gameId in stack_map.keys():
            players_larger = max(len(stack_map[gameId]["homeTeam"]["players"]), len(stack_map[gameId]["awayTeam"]["players"]))
            players_smaller = min(len(stack_map[gameId]["homeTeam"]["players"]), len(stack_map[gameId]["awayTeam"]["players"]))
